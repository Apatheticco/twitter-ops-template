...
**④ 🔑 依赖文件"存在但仍是占位符"= 视同没有**：
本 Skill 依赖 `voice-guide.md` / `audience-profile.md` / `operations-plan.md`，三者出厂**都存在**
但全是 `[…]` 占位或带 `<!-- INIT-STATUS: template-default -->` 状态位 → **一律按未配置处理**。
...
> | §频次预算 日上限 | operations-plan §二 | 全节是 `[X] 条`；`普通日/重大日/突发日` **0 命中** |
...
- **频次预算**：日上限见 `twitter-ops/references/operations-plan.md` §二。
  ...
  此时只在输出附一句「日上限未配置，未做频次检查」，**不得硬叫停，也不得拿默认值当用户的配置**。
  读到了真数字才执行叫停。

...
- **强制**：读 `$STATE_DIR/trend-scout-candidates/$DATE-latest.txt`（`$DATE` 来自 §0 时钟；该文件内容为当次 candidates json 的绝对路径）→ candidates.json（首扫 ≥12 / 刷新 ≥7）。`STATE_DIR` 取自 `config.md`，**与 trend-scout 写入的是同一个目录**。**禁止跳过 json 直接读简报挑话题**。
  ...
  > **不许退而读昨天的**；② 读到 json 后取**顶层** `scan_ts_ms`（trend-scout §5.3 必填字段），
  > `NOW_MS − scan_ts_ms > 6h` → 同样拒绝。字段缺失 → 按「候选池格式不合规」要求重跑 trend-scout，
  > **不许当作"没超时"放行**（缺字段和新鲜是两回事）。
  >
  > **逃生口（和 §2「可用 <2」那条同规格）**：MCP 挂了、限速、或用户就是想用早上那批料时，
  > 允许**显式披露 + 用户确认后放行**——输出顶部写一行
  > 「⚠️ 本轮建在 Nh 前的候选池上（采集于 HH:MM），时效分未按当前时间重算」，
  > 并把受影响的 `base_timeliness` 逐条标出来。**这是"披露后放行"，不是"静默放行"**：
  > 全仓的一贯做法是给逃生口但要求披露，只有「不许」没有出口的硬闸会在真出故障时把人堵死。
  >
...
- **周单主题上限预警**（防单引擎依赖：赢在主题恰好对、抗风险极差，同主题堆量边际递减）：出选题前读本周已发帖落盘，统计**单一主题 / 实体占比**。>60% → 本批优先非该主题 + 软提醒；>80% → 红色提醒 + 本批强制至少 1 条非该主题（除非该主题当日有 `raw ≥4.5` 抢先级 BREAKING）。文件缺 **或仍是占位符** → 跳过（宁缺勿误）。
